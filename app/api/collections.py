...
async def create_collection(
    ...
    **Step 1: Create collection and upload CSV files.**
    ...
    for idx, file in enumerate(files):
        ...
        kedro_path = f"data/01_raw/{project_id}/{original_filename}"
        ...
        # Create Dataset record (so file is independently accessible)
        dataset = Dataset(
            id=file_id,
            name=table_name,
            project_id=project_id,
...
async def add_tables(
    ...
    project_dir = os.path.join(KEDRO_RAW_DATA_DIR, coll.project_id)
    os.makedirs(project_dir, exist_ok=True)
    ...
    for idx, file in enumerate(files):
        ...
        kedro_path = f"data/01_raw/{coll.project_id}/{original_filename}"
...
async def process_collection(
    ...
    **Step 5: Execute the merge pipeline — "Create & Process" button.**
    ...
    try:
        ...
        output_dir = os.path.join(KEDRO_RAW_DATA_DIR, coll.project_id)
        output_path = os.path.join(output_dir, output_filename)
        kedro_output_path = f"data/01_raw/{coll.project_id}/{output_filename}"
        ...
        # Create Dataset record for merged output
